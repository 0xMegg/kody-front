...
export default async function SupabaseShipmentDetailPage({
  params,
}: {
  ...
  return (
    <div>
      <PageHeader
        ...
        title={shipment.id}
        actions={
          <Badge variant={statusVariantMap[shipment.status] ?? "neutral"}>
            {shipment.status}
          </Badge>
        }
      />
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <Card title="기본 정보">
          ...
          >
            ...
            <InfoRow
              label="인코템즈"
              value={<Badge variant="neutral">{shipment.incoterm}</Badge>}
            />
            <InfoRow
              label="출고일"
              value={
                <span style={{ fontFamily: "var(--font-jetbrains-mono)" }}>
                  {shipment.shipDate ?? "미정"}
                </span>
              }
            />
            <InfoRow
                  ...
                  return (
                    <tr key={item.orderItemId}>
                      {[
                        ...
                        { value: item.hsCode, align: "left" as const, mono: true },
                        { value: item.htsCode, align: "left" as const, mono: true },
                      ].map((cell, ci) => (
                        <td
                          key={ci}
                          style={{
