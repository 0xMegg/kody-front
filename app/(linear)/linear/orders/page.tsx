...
function FilterBar({
  ...
  const toggleBtnStyle = (active: boolean): React.CSSProperties => ({
    ...
    borderRadius: 6,
    cursor: "pointer",
    backgroundColor: active ? "var(--l-brand-subtle)" : "transparent",
...
// Main page
// ---------------------------------------------------------------------------
export default function LinearOrdersPage() {
  ...
  return (
    <div>
      <PageHeader
        breadcrumb="Linear / 주문"
        title="주문"
        actions={
          <Button onClick={() => router.push("/linear/orders/new")}>
            새 주문
          </Button>
        }
      />
      ...
      {view === "order" ? (
        <Table<OrderRow>
          ...
          onRowClick={(row: OrderRow) =>
            router.push(`/linear/orders/${row.id}`)
          }
          renderCell={(
        ...
        <Table<OrderItemRow>
          ...
          onRowClick={(row: OrderItemRow) =>
            router.push(`/linear/orders/${row.orderId}#${row.id}`)
          }
