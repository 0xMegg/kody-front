...
import type { Order, Account, AccountBalance } from "@/lib/types";
...
function LinearBalanceCard({ balance }: { balance: AccountBalance }) {
  return (
...
function NotionBalanceCallout({ balance }: { balance: AccountBalance }) {
  return (
    <div
      style={{
...
function SupabaseBalanceCard({ balance }: { balance: AccountBalance }) {
  return (
    <Card title="선택된 거래처 잔액">
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {(["USD", "EUR", "RUB", "KRW"] as const).map((cur) => (
  ...
  return (
    <div style={{ display: "flex", gap: 24, alignItems: "flex-start" }}>
      ...
      >
        ...
        {isNotion ? (
          <NotionBalanceCallout balance={balance} />
        ) : isSupabase ? (
          <SupabaseBalanceCard balance={balance} />
        ) : (
          <LinearBalanceCard balance={balance} />
        )}
